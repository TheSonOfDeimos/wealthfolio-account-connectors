...
export async function applyCryptoComPricing(
  ...
  for (const asset of before.assets) {
    try {
      ...
      if (existing && existing !== QUOTE_PROVIDER.id) {
        conceded.push(`${asset.symbol} (${existing})`);
        continue;
      }
  ...
  if (conceded.length > 0) {
    log(
      'info',
      `${conceded.length} asset(s) are already priced by another connector's provider and were ` +
        `left alone: ${conceded.join(', ')}. Wealthfolio shares one asset across accounts, so ` +
        'whichever connector claimed it first keeps it — both quote the same coin.',
    );
  }
